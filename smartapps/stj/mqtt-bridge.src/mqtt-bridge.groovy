...
//Insert a shim here to change the device values outbound to HS
def inputHandler(evt) {
	log.debug "In inputHandler (outbound)"
    def updValue = evt.value
    log.debug evt.name
    log.debug "updValue before $updValue"
switch (evt.name){
		case "switch":
        	switch (evt.value){
            	case "off": updValue = "0"; break
                case "on": updValue = "100"; break
            }
            break
    	case "thermostatMode":
        	switch (evt.value){
        		case "off": updValue = "0"; break
            	case "heat": updValue = "1"; break
            	case "cool": updValue = "2"; break
            	}
        	break
        case "thermostatFanMode":
             switch (evt.value){
        		case "fanAuto": updValue = "0"; break
            	case "fanOn": updValue = "1"; break
            	}
            break
        case "thermostatOperatingState":    
           	switch(evt.value){
            	case "idle": updValue = "0";break
                case "heating": updValue = "1";break
                case "cooling": updValue = "2";break
	            }
            break    
        }
        	
    log.debug "updValue after $updValue"    
...
log.debug "Switch sent us something"
log.debug  "value = $value"
	def stringValue = null
	switch(value){
    	case "0": device.off();log.debug "off";break
        case "100": device.on();log.debug "on";break
        }
}
...
//Fix this for value translation inbound
def actionThermostat(device, attribute, value) {
	log.debug "In Action Thermostat"
	log.debug "Attribute is $attribute"
    log.debug "value is $value"
    switch(attribute) {
        case "heatingSetpoint":
//Convert the inbound string value to int
			int intvalue = Integer.parseInt(value)
            device.setHeatingSetpoint(intvalue)
            device.setThermostatSetpoint(intvalue)
        break
        case "coolingSetpoint":
        	int intvalue = Integer.parseInt(value)
            log.debug "converted int $intvalue"
            device.setCoolingSetpoint(intvalue)
            device.setThermostatSetpoint(intvalue)
        break
        case "thermostatMode":
        	log.debug "in thermostatMode"
        	def stringvalue = null
             switch(value){
            	case "0":
                	stringvalue = "off"
                ...
                case "1":
                	stringvalue = "heat"
                    device.setThermostatSetpoint(device.currentValue(heatingSetpoint))
                    break
                case "2":
                	stringvalue = "cool"
                    device.setThermostatSetpoint(device.currentValue(coolingSetpoint))
                    break
            }
             log.debug "setting the thermostat mode to $stringvalue"
            device.setThermostatMode(stringvalue)
        break
        case "thermostatFanMode":
        	def stringvalue = null
             switch(value){
            	case "0": stringvalue = "fanAuto";break
                case "1": stringvalue = "fanOn";break
            }
            device.setThermostatFanMode(stringvalue)
        break
        case "temperature":
            int intvalue = Integer.parseInt(value)
        	device.setTemperature(intvalue)
        break
        case "thermostatOperatingState":
        	def stringvalue = null
        	switch(value){
            	case "0": stringvalue = "idle";break
                case "1": stringvalue = "heating";break
                case "2": stringvalue = "cooling";break
            }
            log.debug "setting the operating state to $stringvalue"
        	device.setThermostatOperatingState(stringvalue)
        break    
    }
}
...
def actionThermostatFanMode(device, attribute, value) {
	log.debug ("Actionthermostatmode $value")
